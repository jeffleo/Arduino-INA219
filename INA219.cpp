...
void Adafruit_INA219::wireWriteRegister (uint8_t reg, uint16_t value)
{
  ...
  //**	I2C library implementation		**//
	...
	data[0] = (value >> 8) & 0xFF;
	data[1] = value & 0xFF;
	_i2c->write(ina219_i2caddr, reg, data, 2);
...
void Adafruit_INA219::wireReadRegister(uint8_t reg, uint16_t *value)
{
  ...
  //**	I2C library implementation		**//
	_i2c->write(ina219_i2caddr, reg);

	//JL: ^ slightly less blocking per readReg call
	unsigned long timein = micros();
	while (micros() - timein < 325);		// Max 12-bit conversion time is 586us per sample, 304us for 11bit for INA219

	_i2c->read(ina219_i2caddr, (uint8_t)2);
	// Shift values to create properly formed integer
	*value = ((_i2c->receive() << 8) | _i2c->receive());
	
	//TODO
	//AsyncRxBuffer[buf_index++] = value;					// put address into buffer of addresses
}

void Adafruit_INA219::I2C_async_Rx() {
	//TODO, how to handle two ina? ever current limiting both?
	_i2c->read(ina219_i2caddr, (uint8_t)2);			// read first to increment byte_availble? will it disturb slaves ADC, check INA? 
	if (_i2c->available() >= 2) {
		*(int16_t*)AsyncRxBuffer[buf_index--] = (int16_t)((_i2c->receive() << 8) | _i2c->receive());
	}
}

//TODO, implement void getcurrent(float* ) instead, (cast to int16_t then to float)
void Adafruit_INA219::getCurrent_raw(uint16_t* I) {
	wireReadRegister(INA219_REG_CURRENT, I);
}

/**************************************************************************/
/*! 
    @brief  Configures to Adafruit_INA219 to be able to measure up to 32V and 2A
...
/**************************************************************************/
/*! 
    @brief  Configures to Adafruit_INA219 to be able to measure up to 32V and 1A
...
/**************************************************************************/
/*!
	@brief set device to alibration which uses 11bit precision for current measurement (0.?mA), at the expense of
	only supporting 16V at 400mA max.
*/
/**************************************************************************/
void Adafruit_INA219::setCalibration_16V_400mA_11bit(void) {
...
/**************************************************************************/
/*! 
    @brief  Instantiates a new Adafruit_INA219 class
...
// JL: unused, and not using TwoWire anymore
///**************************************************************************/
///*! 
...
void Adafruit_INA219::init() {
  _i2c->begin();
  _i2c->timeOut(100);	// millisecondsm JL: this is not available in wire lib
  _i2c->pullup(true);	// JL: necessary for new I2c lib, only correct reg for ATmegaX28P chips***

  // todo async 
  buf_index = 0;

  //// Set chip to large range config values to start
  //setCalibration_32V_2A();							// JL: will set our own Cal
}

/**************************************************************************/
/*!
@brief  scan I2C bus for hardware
*/
/**************************************************************************/
void Adafruit_INA219::scanBus(void) {
	_i2c->scan();
}
...
int16_t Adafruit_INA219::getCurrent_raw() {
  ...
  // Sometimes a sharp load will reset the Adafruit_INA219, which will
  // reset the cal register, meaning CURRENT and POWER will
  // not be available ... avoid this by always setting a cal
  // value even if it's an unfortunate extra step
  //wireWriteRegister(INA219_REG_CALIBRATION, ina219_calValue);			// JL: won't happen for us
...
*/
/**************************************************************************/
void Adafruit_INA219::setAddr(uint8_t addr) {
	ina219_i2caddr = addr;
}
